...
router.post('/building-create',AdminVerifyTokenMiddleware,
    uploadImages.fields([{ name: 'm01_image', maxCount: 1 }])
    ,validateDataMiddleWare(buildingCreateSchema),buildingCreateController);
router.put('/building-update/:id',AdminVerifyTokenMiddleware,
    uploadImages.fields([{ name: 'm01_image', maxCount: 1 }]),
    validateDataMiddleWare(buildingUpdateSchema),buildingUpdateController);
